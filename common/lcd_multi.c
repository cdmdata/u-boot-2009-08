...
static void lcd_drawchar8( struct lcd_t *lcd, char c )
{
	...
	if (lcd->drawchar){
		lcd->drawchar(lcd,src,VIDEO_FONT_WIDTH,VIDEO_FONT_HEIGHT);
		return ;
	}
	dest = (uchar *)lcd->fbAddr + (lcd->stride*VIDEO_FONT_HEIGHT*lcd->y) + lcd->x*VIDEO_FONT_WIDTH ;
...
{
	if (curPanel_ < numPanels_) {
                struct lcd_t *panel = panels_[curPanel_];
		if (panel->display_bmp) {
			return panel->display_bmp(panel,bmp,x,y);
		}
		else {
			printf("%s: device does not support bmp display\n", __func__);
			return -1 ;
		}
	}
	else {
		printf("%s:no current panel\n",__func__);
		return -2 ;
	}
}
