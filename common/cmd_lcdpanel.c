...
#if defined( CONFIG_CMD_LCDPANEL )
...
void print_panel_info( struct lcd_panel_info_t const *panel )
{
   ...
   printf( "active         : %u\n", panel->active );
   printf( "CRT            ? %u\n", panel->crt);
   printf( "%s:%lu,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
...
#endif
	char panelname[512];
printf( "%s: panel %s", __func__, panel->name );
...
char* find_set_panel(char* next, int* pmatched)
{
	int matched = 0;
#if nitrogen == BOARD
	struct lcd_panel_info_t writeable_panel ;
#endif
...
#if nitrogen == BOARD
		memcpy(&writeable_panel,panel,sizeof(writeable_panel));
		writeable_panel.name = "mxcfb:raw" ;
		if (set_p(&writeable_panel))
			matched++;
#else
		if (set_p(panel))
			matched++;
...
#if defined(CONFIG_LCD_MULTI) || defined(CONFIG_LCD)
...
static int lcdpanel(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	if ( 1 == argc ) {
		...
		if( panel ) {
			char panelname[512];
#if nitrogen == BOARD
			((struct lcd_panel_info_t *)panel)->name = "mxcfb:raw" ;
#endif
			build_panel_name(panelname, panel);
#if defined(CONFIG_LCD_MULTI)
			if (addPanel(panel, panelname))
			...
			lcd_puts(panelname);
#endif
			printf( "setting panel to dynamic %s\n", panelname);
